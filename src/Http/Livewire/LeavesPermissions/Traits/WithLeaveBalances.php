...
trait WithLeaveBalances
{
    ...
    public array $selectedBalanceSummary = [];
    ...
    protected function recalculateBalance(int $companyId, int $employeeId, int $policyId, int $yearId): void
    {
        ...
        $employee = \Athka\Employees\Models\Employee::withoutGlobalScope('active_only')->find($employeeId);
        ...
        $amounts = $this->calculateLeaveBalanceAmounts($policy, $employee, $taken);
        ...
        AttendanceLeaveBalance::updateOrCreate(
            ...
            [
                'entitled_days' => $amounts['entitled'],
                'taken_days' => $amounts['taken'],
                'remaining_days' => $amounts['remaining'],
    ...
    public function openBalanceAudit(int $balanceId): void
    {
        ...
        $this->selectedBalance->setRelation(
            'employee',
            \Athka\Employees\Models\Employee::withoutGlobalScope('active_only')->find($this->selectedBalance->employee_id)
        );

        $this->balanceHistory = AttendanceLeaveRequest::where('company_id', $this->selectedBalance->company_id)
            ->where('employee_id', $this->selectedBalance->employee_id)
        ...
        $this->selectedBalanceSummary = $this->calculateLeaveBalanceAmounts(
            $this->selectedBalance->policy,
            $this->selectedBalance->employee,
            (float) $this->balanceHistory->sum('requested_days')
        );
    ...
    protected function calculateLeaveBalanceAmounts(?LeavePolicy $policy, $employee, float $taken): array
    {
