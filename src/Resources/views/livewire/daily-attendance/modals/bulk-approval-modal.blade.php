        ...
        <x-slot name="content">
            <div class="space-y-4">
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-2 gap-4 border-t border-gray-200 pt-4">
                        <div class="text-center">
                            <div class="text-2xl font-bold text-[color:var(--success)]">{{ $bulkSummary['ready_count'] ?? 0 }}</div>
                            <div class="text-xs text-gray-500">{{ tr('Ready to Approve') }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl font-bold {{ ($bulkSummary['issues_count'] ?? 0) > 0 ? 'text-[color:var(--warning)]' : 'text-gray-400' }}">
                ...
                @if(($bulkSummary['issues_count'] ?? 0) > 0)
                    <div class="p-3 bg-[color:var(--warning)]/10 border border-[color:var(--warning)]/25 rounded-lg flex items-start gap-3">
                        <i class="fas fa-exclamation-triangle text-[color:var(--warning)] mt-0.5"></i>
                ...
                @else
                    <div class="p-3 bg-[color:var(--success)]/10 border border-[color:var(--success)]/25 rounded-lg flex items-start gap-3">
                        <i class="fas fa-check-circle text-[color:var(--success)] mt-0.5"></i>
                        <div class="text-sm text-[color:var(--text-primary)]">
